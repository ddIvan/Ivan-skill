...
public class AuthController : ControllerBase
{
    ...
    /// <summary>登录</summary>
    [HttpPost("login")]
    public ApiResult<LoginResult> Login(LoginRequest request)
    {
        var result = _authService.Login(request);
        return ApiResult<LoginResult>.Ok(result, "登录成功");
    }
    ...
    [Authorize]
    [HttpGet("profile")]
    public ApiResult<LoginResult> Profile()
    {
        var userName = User.FindFirstValue(ClaimTypes.Name) ?? "";
        var result = _authService.GetProfile(userName);
        return ApiResult<LoginResult>.Ok(result);
    }
}
